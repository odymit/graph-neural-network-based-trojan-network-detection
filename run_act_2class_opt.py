...
def evaluate(dataloader, device, model, base_model):
    model.eval()
    total = 0
    total_correct = 0
    for i, data in enumerate(tqdm(dataloader)):
        ...
        troj_y_in = None
        if len(data) > 2:
            x_in, _, troj_y_in= data
        else:
            x_in, _ = data
            troj_y_in = torch.tensor(0)
        if i > 2 :
            break
        label = troj_y_in
        ...
        total += 1
        logits = model(message_passed_g, feat.view(len(feat), -1))
        _, predicted = torch.max(logits, 1)
        total_correct += (predicted == label).item()
    acc = 1.0 * total_correct / total
    return acc, total


def train_2class_classification_model(model, base_model, dataloader, testloader_benign, testloader_mal, epoch_num, device):
    ...
    for epoch in range(epoch_num):
        ...
        break
        # scheduler.step()
        # train_acc = evaluate(trainloader, device, model, base_model)
        # valid_acc_benign = evaluate(testloader_benign, device, model, base_model)
        # valid_acc_mal = evaluate(testloader_mal, device, model, base_model)
        # print(
        #     "Epoch {:05d} | Loss {:.4f} | Train Acc. {:.4f} | Benign - Validation Acc. {:.4f} | Mal. - Validation Acc. {:.4f} ".format(
        #         epoch, total_loss, train_acc, valid_acc_benign, valid_acc_mal
        #     )
        # )
        # print(epoch, total_loss, train_acc, valid_acc_benign, valid_acc_mal)
...
if __name__ == '__main__':
    ...
    trainloader = torch.utils.data.DataLoader(trainset_mal, batch_size=1, shuffle=True)
    ...
    testloader_benign = torch.utils.data.DataLoader(testset, batch_size=1)
    # testloader_mal = torch.utils.data.DataLoader(testset_mal, batch_size=BATCH_SIZE)
    testloader_mal = torch.utils.data.DataLoader(testset_mal, batch_size=1)
    ...
    #cProfile.run('train_2class_classification_model(model, base_model, trainloader, testloader_benign, testloader_mal, 3, device)')
    train_2class_classification_model(model, base_model, trainloader, testloader_benign, testloader_mal, 3, device)

    
    #     train_model(model, trainloader, epoch_num=N_EPOCH, is_binary=is_binary, verbose=False)
    # save_path = SAVE_PREFIX+'/models'+args.model+'/two-class.pt'
